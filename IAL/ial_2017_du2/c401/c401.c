...
**/
    ...
    {
        (*RootPtr) = calloc(sizeof(struct tBSTNode), 1);
        if ((*RootPtr) != NULL)
        {
            (*RootPtr)->Key = K;
            (*RootPtr)->BSTNodeCont = Content;
...
**/
    if ((*RootPtr) == NULL) // chyba
        return;
    if ((*RootPtr)->RPtr == NULL) // tento je nepravejsi
    {
        tBSTNodePtr toCopy = (*RootPtr);
        PtrReplaced->Key = toCopy->Key;
        PtrReplaced->BSTNodeCont = toCopy->BSTNodeCont;
        if ((*RootPtr)->LPtr != NULL) // pokud to neni list
            PtrReplaced->LPtr = toCopy->LPtr;
        free (toCopy);
        toCopy = NULL;
    }
    else
    {
        ...
        ReplaceByRightmost(PtrReplaced, &tmp);
    ...
    if ((*RootPtr) != NULL)
    {
        ...
        else // tenhle zrus
        {
            ...
            else
            {
                ...
                ReplaceByRightmost((*RootPtr), tmp);
            }
        }
    }
}
...
**/
    if ((*RootPtr) == NULL)
        return;
    BSTDispose (&((*RootPtr)->RPtr)); // nejdrive se vymazou vsechny prave pak az leve
    BSTDispose (&((*RootPtr)->LPtr));

    free ((*RootPtr));
    // nastav na NULL aby se nikdo nepokousel pristoupit
    (*RootPtr) = NULL;
}
